...
# Load Census Data
dat<-read.table('transfer-2013.csv',sep=',',header=T,na.strings = 'N')
dat$state = tolower(dat$state)
dat$overweight<-dat$Overweight.2013
dat$obese<-dat$Obese.2013
head(dat)
...
text.labels$outside=text.labels$id %in% c('MA','RI','CT','NJ','DE','MD')
...
map<-ggplot(lnd_f)+
  ...
  scale_fill_manual(values=colors,
                      guide = guide_legend(title='Percentage of Obese Adult Population',
                                           title.position='top',
...
png('/home/roberto/Documents/freelance/oDesk/obesity-remake-example2.png',height=1200,width=1200)
map
dev.off()


lnd_b3<-ggmap(get_map(location=b,source='google',maptype='satellite',crop=TRUE,zoom=4,))
map<-lnd_b3+
  theme_bw()+
  geom_polygon(data=lnd_f,aes(x = long, y = lat,group = group, fill = obeseFactor),alpha=0.6)+ 
  geom_polygon(data=lnd_f,aes(x = long, y = lat,group = group), fill=NA,colour='grey', size=.8)+
  #scale_fill_continuous(name='Obesity',low='#aaffaa',high='#ffaaaa')+
  scale_fill_manual(values=colors,
                    guide = guide_legend(title='Percentage of Obese Adult Population',
                                         title.position='top',
  ...
  #scale_fill_continuous(guide = guide_legend())+
  #coord_map(project="conic", lat0 = 30)+
  geom_text(data = text.labels,aes(x = x,y=y,label=lab),colour='white',size=7) +
  ...
  theme(axis.ticks = element_blank(), 
        ...
        legend.title=element_text(size=20),
        legend.text=element_text(size=18))
png('/home/roberto/Documents/freelance/oDesk/google-hybrid-obesity-overlay.png',height=1200,width=1200)
map
dev.off()
...
#
cities<-read.table('~/Documents/bertplot/data/cities/uscities.clean.csv',sep=',',header=F,na.strings = '',)
colnames(cities)<-c('Country','city','AccentCity','state','Population','Latitude','Longitude')

cities<-cities[!is.na(cities$Population) & !is.na(cities$Latitude) & !is.na(cities$Longitude) & !cities$state %in% c('AK','HI') ,]
spatial.inner <- with(cities, SpatialPointsDataFrame(coords = cbind(Longitude,Latitude),data=cities))

b <- bbox(states50)
b[1, ] <- c(-160,-30)
b[2, ] <- c(30,60)

df.inner<-data.frame(spatial.inner)
df.inner$point.size = 30* df.inner$Population / max(df.inner$Population)
#lnd_b3<-ggmap(get_map(location=b,zoom=4))
png('/home/roberto/Documents/freelance/oDesk/google-terrain-just-metro-micro.png',height=1200,width=1200)
lnd_b3<-ggmap(get_map(location='usa',zoom=4))
lnd_b3+ geom_point(data=df.inner,aes(Longitude,Latitude),alpha=0.5,size=1.3)+guides(fill=FALSE)
dev.off()
#
# Density
#
sSp <- as(SpatialPoints(spatial.inner), "ppp") 
Dens <- density(sSp, adjust = 0.2,) 
Dsg<-as(Dens,'SpatialGridDataFrame')
Dim<-as.image.SpatialGridDataFrame(Dsg)
Dcl<-contourLines(Dim,nlevels=8)
SLDF<-ContourLines2SLDF(Dcl,CRS("+proj=longlat +ellps=GRS80 +datum=NAD83 +no_defs +towgs84=0,0,0"))

#spatial.inner<-spTransform(spatial.inner,CRS("+proj=longlat +ellps=GRS80 +datum=NAD83 +no_defs +towgs84=0,0,0"))
...
lnd_b3<-ggmap(get_map(location='usa',zoom=4,maptype='hybrid'))
png('/home/roberto/Documents/freelance/oDesk/google-hybrid-city-density-heatmap.png',height=1200,width=1200)
lnd_b3+  
  theme_bw()+
  geom_polygon(data=x,aes(long,lat,group=group,fill=id),alpha=0.7)+
  geom_point(data=df.inner,aes(Longitude,Latitude),alpha=0.5,colour='yellow',size=1.3)+
  theme(axis.ticks = element_blank(), 
        axis.text.x = element_blank(),
        axis.text.y = element_blank(),
        axis.title.x= element_blank(),
